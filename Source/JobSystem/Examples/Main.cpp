...
void Example_IndependentTasks()
{
	...
	std::shared_ptr<TaskEvent> taskA = Task::CreateAndDispatch(
		[]()
		{
	...
	std::shared_ptr<TaskEvent> taskB = Task::CreateAndDispatch(
		[]()
		{
	...
	std::shared_ptr<TaskEvent> taskC = Task::CreateAndDispatch(
		[]()
		{
...
void Example_TaskChain()
{
	...
	std::shared_ptr<TaskEvent> task1 = Task::CreateAndDispatch(
		[]()
		{
	...
	std::shared_ptr<TaskEvent> task2 = Task::CreateAndDispatch(
		[]()
		{
	...
	std::shared_ptr<TaskEvent> task3 = Task::CreateAndDispatch(
		[]()
		{
...
void Example_ForkJoin()
{
	...
	std::shared_ptr<TaskEvent> joinTask = Task::CreateAndDispatch(
		[]()
		{
...
void Example_NestedTasks()
{
	...
	std::shared_ptr<TaskEvent> parentTask = Task::CreateAndDispatch(
		[]()
		{
			std::cout << "Parent task started\n";

			std::shared_ptr<TaskEvent> childA = Task::CreateAndDispatch(
				[]()
				{
					std::cout << "	Child A executing\n";
					std::this_thread::sleep_for(std::chrono::milliseconds(50));
				});
			std::shared_ptr<TaskEvent> childB = Task::CreateAndDispatch(
				[]()
				{
...
void Example_ParallelProcessing()
{
	...
	{
		std::shared_ptr<TaskEvent> task = Task::CreateAndDispatch(
			[i, &results]()
			{
	...
	std::shared_ptr<TaskEvent> aggregateTask = Task::CreateAndDispatch(
		[&results]()
		{
			int32_t total = 0;
